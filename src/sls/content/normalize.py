"""Backend-neutral normalization of Original and simulator content identifiers."""
...
def normalize_content_id(value: object) -> str:
    text = re.sub(r"(?<=[a-z0-9])(?=[A-Z])", "_", str(value or "UNKNOWN"))
    normalized = "_".join(text.upper().replace("_", " ").replace("-", " ").split())
    return {
