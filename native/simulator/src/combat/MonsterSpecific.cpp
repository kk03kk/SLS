...
void Monster::preBattleAction(BattleContext &bc) {
    ...
    switch (id) {
        ...
        case MonsterId::GIANT_HEAD:     // game adds slow power
            setStatus<MS::SLOW>(0);
            // Slow is an active zero-amount power at combat start. setStatus(0)
            // normally removes a power, so restore the presence bit after
            // initializing its shared amount storage.
        ...
        case MonsterId::SPIRE_SHIELD: {
            // A room transition resets AbstractPlayer.flipHorizontal.  The
            // default orientation faces the Spear (slot 1), leaving Shield as
            // the initial BackAttack source.
            bc.player.lastTargetedMonster = 1;
    ...
    switch (moveHistory[0]) {
      ...
      assert(curHp > 0);
        ...
        case MMID::SPHERIC_GUARDIAN_ACTIVATE: // 2
            ...
            bc.addToBot(Actions::SetMoveAndNoOpRoll(
                idx, MMID::SPHERIC_GUARDIAN_ATTACK_DEBUFF));
            break;
        ...
        case MMID::SPHERIC_GUARDIAN_ATTACK_DEBUFF: // 4
            ...
            bc.addToBot(Actions::SetMoveAndNoOpRoll(
                idx, MMID::SPHERIC_GUARDIAN_SLAM));
            break;
        ...
        case MMID::SPHERIC_GUARDIAN_SLAM: // 1
            ...
            bc.addToBot(Actions::SetMoveAndNoOpRoll(
                idx, MMID::SPHERIC_GUARDIAN_HARDEN));
            break;
        ...
        case MMID::TASKMASTER_SCOURING_WHIP: // todo buff after calculating damage so no need to add to bot?
            ...
            if (asc18) {
                ...
                // Java queues Damage, Wounds, then Strength at A18+.
                // Keeping the same action order matters to queue parity.
        ...
        case MMID::THE_CHAMP_GLOAT: {
            // Champ.strAmt is 2/3/4 at A0/A4/A19.  Anger triples this
            // amount, but Gloat applies it once.
        ...
        case MMID::TRANSIENT_ATTACK: {
            ...
            // The original Transient bypasses rollMove after attacking and calls
            // setMove directly.  Consuming aiRng here shifts every later monster
            // roll in the room even though Transient's intent is deterministic.
            setMove(MMID::TRANSIENT_ATTACK);
        ...
        case MMID::GIANT_HEAD_IT_IS_TIME: { // 2
            const auto firstAttackTurn = asc18 ? 4 : 5;
            const auto t = std::min(
                    bc.getMonsterTurnNumber() - firstAttackTurn, 6) * 5;
        ...
        case MMID::SPIRE_SHIELD_FORTIFY: // 2
            addBlock(30);
            if (!bc.monsters.arr[1].isDeadOrEscaped()) {
        ...
        case MMID::CORRUPT_HEART_BLOOD_SHOTS: // 1
            ...
            // AbstractMonster queues its RollMoveAction after the attack.  If
            // Blood Shots is lethal that queued action never runs, so consuming
            // the AI roll inline would drift the terminal Original RNG state.
            bc.addToBot(Actions::NoOpRollMove());
            break;
        ...
        case MMID::CORRUPT_HEART_ECHO: // 2
            ...
            // Keep the future-move RNG action behind the damage action.  The
            // action manager stops on player death, matching Original's final
            // Heart-loss boundary without an extra AI RNG consumption.
            bc.addToBot(Actions::NoOpRollMove());
            break;
...
MMID Monster::getMoveForRoll(BattleContext &bc, int &monsterData, const int roll) const {
    ...
    switch (id) {
        ...
        case MonsterId::BRONZE_ORB: {
            ...
            // 2 support beam
            // 3 stasis
            auto &haveUsedStasis = monsterData;
...
//            }
        ...
        case MonsterId::NEMESIS: {
            ...
            // 3 scythe
            // 4 debuff
            auto &scytheCooldown = monsterData;
            --scytheCooldown;
            ...
            if (roll < 30) {
                if (!lastMove(MMID::NEMESIS_SCYTHE) && scytheCooldown <= 0) {
                    scytheCooldown = 2;
            ...
            if (bc.aiRng.randomBoolean() && scytheCooldown <= 0) {
                scytheCooldown = 2;
                return MMID::NEMESIS_SCYTHE;
            }
        ...
        case MonsterId::REPTOMANCER: {
            ...
            int myRoll = roll;
            // Java Reptomancer.canSpawn counts the other living monsters and
            // permits three of them, i.e. four total living entities including
            // Reptomancer.  At A18 a summon can fill just the final open
            // dagger slot; it is not all-or-nothing.
        ...
        case MonsterId::THE_CHAMP: {
            ...
            // monsterData stores forgeTimes in its low two bits and the
            // phase-two flag in bit 2.  Reading miscInfo here made the
            // two-use Defensive Stance cap ineffective.
            const auto defensiveStanceUseCount = monsterData & 0x3;
...
void Monster::largeSlimeSplit(BattleContext &bc, const MonsterId mediumSlimeType, const int placeIdx, const int hp) {
    ...
    auto splitLarge = bc.monsters.arr[placeIdx];
    splitLarge.curHp = 0;
    splitLarge.block = 0;
    ...
    // SpikeSlime_L queues RollMoveAction unconditionally after its switch, so
    // its dead split entity consumes one final AI value. AcidSlime_L queues
    // RollMoveAction only in its three non-split cases; its split case neither
    // consumes AI RNG nor replaces the UNKNOWN split intent.
    if (mediumSlimeType == MonsterId::SPIKE_SLIME_M) {
        splitLarge.rollMove(bc);
    }
    if (bc.encounter == MonsterEncounter::LARGE_SLIME) {
        // SpawnMonsterAction retains the dead parent in the Java MonsterGroup
        // between its two spawned medium slimes.  Keep it in an unused native
        // slot so the bridge can project that public entity without making it
        // targetable or part of the turn loop.
        splitLarge.idx = 4;
        bc.monsters.arr[4] = splitLarge;
    } else if (bc.encounter == MonsterEncounter::SLIME_BOSS) {
        const int ghostIdx = mediumSlimeType == MonsterId::SPIKE_SLIME_M ? 4 : 5;
        splitLarge.idx = ghostIdx;
        bc.monsters.arr[ghostIdx] = splitLarge;
    }
    bc.monsters.extraRollMoveOnTurn.set(idx2, false);
    bc.monsterTurnIdx++;
}
void Monster::slimeBossSplit(BattleContext &bc, const int hp) {
    ...
    // The Java MonsterGroup keeps the dead Slime Boss object in the middle
    // after inserting the two large slimes.  Preserve that public entity (and
    // its original max HP/identity) instead of exposing the default INVALID
    // hole used by the compact simulator layout.
    auto splitBoss = bc.monsters.arr[0];
    splitBoss.idx = 1;
    splitBoss.curHp = 0;
    splitBoss.block = 0;
    splitBoss.idx = 6;
    bc.monsters.arr[6] = splitBoss;
...
void Monster::reptomancerSummon(BattleContext &bc, int daggerCount) {
    ...
    for (int i = 0; i < daggerCount; ++i) {
        ...
        bc.recordReusedSummonGhost(dagger, daggerIdx);
        dagger = Monster();
