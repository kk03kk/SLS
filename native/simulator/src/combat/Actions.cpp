...
// Ownerless/THORNS-style damage intentionally bypasses player attack hooks
// while retaining monster HP-loss state changes in Monster::damage.
Action Actions::DamageAllEnemy(int damage) {
    return {[=] (BattleContext &bc) {
...
Action Actions::LoseEnergy(int amount) {
    return {[=] (BattleContext &bc) {
        bc.player.energy = std::max(0, bc.player.energy - amount);
    }, false};
}
...
Action Actions::MonsterGainBlock(int idx, int amount) {
    return {[=] (BattleContext &bc) {
        // GainBlockAction does nothing when its target died between queueing
        // and execution. This matters for Curl Up followed by a lethal later
        // hit from the same multi-hit card.
        Monster &monster = bc.monsters.arr[idx];
...
Action Actions::MonsterRemoveCurlUp(int idx) {
    return {[=] (BattleContext &bc) {
        Monster &monster = bc.monsters.arr[idx];
        // RemoveSpecificPowerAction also skips dead targets. This deliberately
        // leaves Curl Up observable until the next agent-boundary cleanup.
        if (!monster.isDeadOrEscaped()) {
            monster.setHasStatus<MS::CURL_UP>(false);
            monster.curlUpTriggered = false;
        }
    }};
}
...
Action Actions::ReactiveRollMove() {
    return {[=] (BattleContext &bc) {
        ...
        // Stock ReactivePower has no amount/counter semantics: every
        // qualifying non-lethal hit queues exactly one RollMoveAction, and
        // the power remains installed.  Its native amount is intentionally
        // zero, so looping over that amount suppressed every reactive roll
        // and setStatus(0) also removed the power after the first hit.
        m.rollMove(bc);
    }};
}
...
Action Actions::SetMoveAndNoOpRoll(int monsterIdx, MonsterMoveId move) {
    return {[=] (BattleContext &bc) {
        bc.monsters.arr[monsterIdx].setMove(move);
        bc.noOpRollMove();
    }};
}
...
Action Actions::PlayTopCardRandomTarget(bool exhausts) {
    return {[=](BattleContext &bc) {
        // MayhemPower first queues an anonymous action; that action chooses
        // its target only when it runs, after earlier start-turn actions may
        // have killed or escaped a monster.
        const auto target = bc.monsters.getRandomMonsterIdx(bc.cardRandomRng, true);
        bc.addToBot(PlayTopCard(target, exhausts));
    }};
}
...
Action Actions::DiscardNoTriggerCard() {
    return {[](BattleContext &bc) {
        ...
        const auto &item = bc.curCardQueueItem;
        const bool spoonProc = item.exhaustOnUse &&
...
Action Actions::SummonGremlins() {
    ...
    return {[=] (BattleContext &bc) {
        ...
        bc.recordGremlinLeaderGhost(gremlin0, newGremlinIdxs[0]);
        bc.recordGremlinLeaderGhost(gremlin1, newGremlinIdxs[1]);
        gremlin0 = Monster();
        gremlin1 = Monster();
...
Action Actions::SpawnTorchHeads() {
    return {[=] (BattleContext &bc) {
        ...
        for (int i = 0; i < spawnCount; ++i) {
            ...
            bc.recordReusedSummonGhost(torchHead, idx);
            torchHead = Monster();
...
Action Actions::DiscoveryAction(CardType type, int amount) {
    return {[=] (BattleContext &bc) {
        ...
        bc.openDiscoveryScreen(sts::generateDiscoveryCards(bc.cardRandomRng, bc.player.cc, type), amount, type);
    }};
}
...
Action Actions::BetterDiscardPileToHandAction(int amount, CardSelectTask task) {
    return {[=] (BattleContext &bc) {
        ...
        if (bc.cards.discardPile.size() <= amount) {
            while (!bc.cards.discardPile.empty() && bc.cards.cardsInHand < CardManager::MAX_HAND_SIZE) {
...
Action Actions::EssenceOfDarkness(int darkOrbsPerSlot) {
    return {[=] (BattleContext &bc) {
        ...
        for (int i = 0; i < count; ++i) bc.player.channelOrb(bc, Orb::DARK);
    }};
}

Action Actions::IncreaseOrbSlots(int count) {
    return {[=] (BattleContext &bc) { bc.player.increaseOrbSlots(count); }};
}
...
Action Actions::ExhaustMany(int limit) {
    return {[=] (BattleContext &bc) {
        // ExhaustAction exits immediately for an empty hand.  Opening a
        // native selector here would add a nonexistent Java UI boundary for
        // Elixir, Purity, and any future user of this helper.
        if (bc.cards.cardsInHand == 0) {
            return;
        }
        bc.inputState = InputState::CARD_SELECT;
...
Action Actions::ToolboxAction() {
    return {[] (BattleContext &bc) {
        ...
        // ChooseOneColorless generates only while opening the screen.  Unlike
        // DiscoveryAction it does not regenerate (and discard) choices when
        // the player retrieves the selected card.
        bc.cardSelectInfo.discoveryCardType = CardType::STATUS;
        bc.cardSelectInfo.discoveryRerollOnRetrieve = false;
...
Action Actions::HandOfGreedAction(int idx, int damage, bool upgraded) {
    return {[=] (BattleContext &bc) {
        ...
        // GreedAction uses a NORMAL DamageInfo sourced by the player.  It must
        // therefore run the attack pipeline (Plated Armor, Curl Up, Envenom,
        // The Boot, etc.), not the non-attack damage pipeline.
        bc.monsters.arr[idx].attacked(bc, damage);
