...
void CardInstance::triggerOnEndOfTurnForPlayingCard(BattleContext &bc) const {
    if (id == CardId::PRIDE) {
        bc.addToBot(Actions::MakeTempCardInDrawPile(*this, 1, false));
        return;
    }
    ...
    switch (id) {
        ...
        case CardId::REGRET: {
            ...
            item.regretCardCount = bc.cards.cardsInHand;
            item.card = *this;
...
bool CardInstance::isFreeToPlay(const BattleContext &bc) const {
    return freeToPlayOnce || (
        getType() == CardType::ATTACK
        && bc.player.hasStatus<PS::FREE_ATTACK_POWER>()
    );
}
