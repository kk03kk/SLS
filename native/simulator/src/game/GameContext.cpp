...
GameContext::GameContext(CharacterClass cc, std::uint64_t seed, int ascension)
    ...
    // The original MainMusic constructor chooses one of two Act tracks with
    // AbstractDungeon.miscRng. Audio is not simulated here, but consuming the
    // same value is required for gameplay RNG parity at the first decision.
    miscRng.random(1);
...
void GameContext::transitionToAct(int targetAct) {
    ...
    if (targetAct == 2 || targetAct == 3) {
        *map = Map::fromSeed(seed, ascension, targetAct, !hasKey(Key::EMERALD_KEY));
        // TheCity/TheBeyond call changeBGM after generating their map. Their
        // MainMusic variants consume miscRng.random(1), just as Exordium does
        // at run start. TheEnding has a single fixed track and consumes none.
        miscRng.random(1);
    } else if (targetAct == 4) {
        *map = Map::act4Map();
    }
...
void GameContext::enterAct3VictoryRoom() {
    ++floorNum;
    // TheEnding is entered on its own floor. AbstractDungeon recreates the
    // per-floor combat streams before exposing the Act 4 map, even though no
    // ordinary map node has been selected yet.
    const auto r = Random(seed + floorNum);
    aiRng = r;
    miscRng = r;
    monsterHpRng = r;
    ...
    switch (curRoom) {
        case Room::MONSTER: {
            ...
            if (info.stolenGold != 0) {
                reward.gold[1] = reward.gold[0];
                reward.gold[0] = info.stolenGold;
                ++reward.goldRewardCount;
            }
        ...
        case Room::EVENT:
            if (curEvent == Event::MYSTERIOUS_SPHERE) {
                Rewards reward;
                addGoldReward(reward, info.gold);
...
//    std::cout << *this << std::endl;
    ...
    if (relics.has(r)) {
        if (r == RelicId::CIRCLET || r == RelicId::RED_CIRCLET) {
            ++relics.getRelicValueRef(r);
        }
        return false;
    }
    ...
    switch (r) {
        ...
        case RelicId::DU_VU_DOLL: {
            relicData = deck.getCountMatching([](const auto &card) {
                return card.getType() == CardType::CURSE;
            });
            break;
        }
...
void GameContext::loseRelic(RelicId r) {
    if (!relics.has(r)) {
        return;
    }
    switch (r) {
        case RelicId::BOTTLED_FLAME:
            deck.removeBottle(CardType::ATTACK);
            break;
        case RelicId::BOTTLED_LIGHTNING:
            deck.removeBottle(CardType::SKILL);
            break;
        case RelicId::BOTTLED_TORNADO:
...
int GameContext::getRandomPlayerNonBasicCardIdx() {
    ...
    // Java keeps the actual AbstractCard object selected by the shuffle.  A
    // deck position is its identity here: collapsing equal-looking copies to
    // the first matching card changes which bottle (if any) is removed.
    return cardIdxs[0];
}
...
void GameContext::addGoldReward(Rewards &r, int amount) const {
    // RewardItem applies Golden Idol whenever it is constructed outside an
    // ordinary TreasureRoom.  Boss treasure is intentionally not excluded:
    // TreasureRoomBoss extends AbstractRoom directly in the Java game.
    if (curRoom != Room::TREASURE && hasRelic(RelicId::GOLDEN_IDOL)) {
        amount += std::round(static_cast<float>(amount) * 0.25f);
    }
    r.addGold(amount);
}
...
void GameContext::openTreasureRoomChest() {
    ...
    if (hasRelic(RelicId::NLOTHS_HUNGRY_FACE) && relics.getRelicValue(RelicId::NLOTHS_HUNGRY_FACE) > 0) {
        // NlothsMask.setCounter(0) immediately calls usedUp(), whose public
        // counter is -2.  Preserve that stock-visible terminal counter rather
        // than leaving the internal charge at zero.
        relics.getRelicValueRef(RelicId::NLOTHS_HUNGRY_FACE) = -2;
    } else {
        reward.addRelic(relic);
    }
...
void GameContext::playerOnDie() {
    ...
    if (it != potions.end()) {
        *it = Potion::EMPTY_POTION_SLOT;
        ...
        curHp = std::max(
            1,
            static_cast<int>((hasBark ? 0.6f : 0.3f) * static_cast<float>(maxHp))
        );
        return;
    }
...
void GameContext::drinkPotion(Potion p) {
    ...
    if (relics.has(RelicId::TOY_ORNITHOPTER)) {
        playerHeal(5);
    }
}
    ...
    switch (curEvent) {
        ...
        case Event::CURSED_TOME: {
            ...
            switch (idx) {
                ...
                case 5: {
                    ...
                    fixed_list<RelicId, 3> possibleBooks;
                    if (!hasRelic(RelicId::NECRONOMICON)) {
                        possibleBooks.push_back(RelicId::NECRONOMICON);
                    }
                    if (!hasRelic(RelicId::ENCHIRIDION)) {
                        possibleBooks.push_back(RelicId::ENCHIRIDION);
                    }
                    if (!hasRelic(RelicId::NILRYS_CODEX)) {
                        possibleBooks.push_back(RelicId::NILRYS_CODEX);
                    }
                    const auto book = possibleBooks.empty()
                        ? RelicId::CIRCLET
                        : possibleBooks[miscRng.random(possibleBooks.size() - 1)];
        ...
        case Event::DEAD_ADVENTURER: { // todo map onto unique idxs
            if (idx == 0) {
                ...
                if (didEncounter) {
                    ...
                    regainControlAction = [=](GameContext &gc) {
                        Rewards reward;
                        gc.addGoldReward(reward, goldAmt);
                        if (addRelic) {
        ...
        case Event::MASKED_BANDITS: {
            if (idx == 0) {
                ...
                const RelicId relicReward = hasRelic(RelicId::RED_MASK)
                    ? RelicId::CIRCLET : RelicId::RED_MASK;
                regainControlAction = [goldAmt, relicReward](auto &gc) {
                    Rewards reward;
                    gc.addGoldReward(reward, goldAmt);
                    reward.addRelic(relicReward);
        ...
        case Event::HYPNOTIZING_COLORED_MUSHROOMS: {
            if (idx == 0) {
                ...
                const RelicId relicReward = hasRelic(RelicId::ODD_MUSHROOM)
                    ? RelicId::CIRCLET : RelicId::ODD_MUSHROOM;
                regainControlAction = [=](GameContext &gc) {
                    Rewards reward;
                    gc.addGoldReward(reward, goldAmt);
                    reward.addRelic(relicReward);
        ...
        case Event::NLOTH: {
            switch (idx) {
                case 0:
                    if (hasRelic(RelicId::NLOTHS_GIFT)) {
                        obtainRelic(RelicId::CIRCLET);
                    } else {
        ...
        case Event::SCRAP_OOZE:
            if (idx == 0) {
                // Java's ScrapOoze increments its dmg after every failed
                // reach attempt, so each subsequent attempt costs one more HP.
        ...
        case Event::THE_MAUSOLEUM: {
            if (idx == 0) {
                ...
                obtainRelic(returnRandomScreenlessRelic(
                    returnRandomRelicTier(relicRng, act)
                ));
                regainControl();
