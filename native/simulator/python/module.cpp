...
Event parse_event(const std::string &name) {
    ...
    for (std::size_t i = 1; i < count; ++i) {
        if (normalized(eventIdStrings[i]) == wanted
                || normalized(eventGameNames[i]) == wanted) {
            return static_cast<Event>(i);
        }
    }
    if (wanted == "COLOSSEUM") return Event::COLOSSEUM;
    if (wanted == "GHOSTS") return Event::GHOSTS;
...
py::list monster_powers(const Monster &m, const BattleContext *context = nullptr) {
    ...
    for (const auto &[status, amount] : m.orderedPowers()) {
        // These are native control markers, not stock AbstractPower entries.
        // Lagavulin's sleep is represented by its SLEEP intent; minion-leader
        // ownership is likewise internal and never appears in CommunicationMod.
        if (status == MonsterStatus::ASLEEP
                || status == MonsterStatus::MINION_LEADER) {
            continue;
        }
...
public:
    ...
    py::dict relic_card_use_probe(
        ...
        bool callback_exhaust = false;
        bool callback_duplicated = false;
        for (int index = 0; index < repetitions; ++index) {
            const std::string current_card_id = relic == RelicId::ORANGE_PELLETS
                ? hand[index] : card_id;
            CardInstance card(parse_card(current_card_id));
            bc_->curCardQueueItem = CardQueueItem(card, 0, bc_->player.energy);
            switch (card.getType()) {
                case CardType::ATTACK:
                    ++bc_->player.attacksPlayedThisTurn;
                    bc_->onUseAttackCard();
                    break;
                case CardType::SKILL:
                    ++bc_->player.skillsPlayedThisTurn;
                    bc_->onUseSkillCard();
                    break;
                case CardType::POWER:
                    bc_->onUsePowerCard();
                ...
                case CardType::CURSE:
                    bc_->onUseStatusOrCurseCard();
                    break;
                default:
                    throw std::logic_error("unsupported relic probe card type");
            }
            callback_exhaust = callback_exhaust || bc_->curCardQueueItem.exhaustOnUse;
        ...
        result["energy_bonus"] = bc_->player.energy - energy_before;
        ...
        result["exhaust"] = callback_exhaust
            || static_cast<int>(bc_->cards.exhaustPile.size()) > exhaust_before;
        std::string zero_card;
        if (relic == RelicId::MUMMIFIED_HAND) {
            for (int index = 0; index < bc_->cards.cardsInHand; ++index) {
                const auto &card = bc_->cards.hand[index];
                if (card.costForTurn == 0) {
                    zero_card = cardStringIds[static_cast<int>(card.id)];
                }
            }
        }
    ...
    py::dict relic_turn_state_probe(
        ...
        if (relic == RelicId::ANCIENT_TEA_SET) {
            ...
            result["energy_delta"] = result["skill_bonus"];
        } else {
    ...
    py::dict relic_trigger_probe(
        ...
        if (relic == RelicId::CHAMPION_BELT) {
            // ChampionsBelt.onTrigger(target) queues Weak only.  The ordinary
            // gameplay path reaches this callback while Vulnerable is being
            // applied, but this probe intentionally isolates the callback.
            bc_->debuffEnemy<MS::WEAK>(0, 1, false);
            bc_->inputState = InputState::EXECUTING_ACTIONS;
            bc_->executeActions();
            ...
            bc_->player.curHp = 1;
            bc_->player.maxHp = 80;
            // Stock's isolated onTrigger probe starts from zero HP.  Keep the
            // simulator alive at one HP to enter its normal death hook, while
            // measuring against the same zero-HP callback boundary.
            hp_before = 0;
...
public:
    ...
    void reset_event_probe(
        ...
        if (gc_->curEvent == Event::MASKED_BANDITS) {
            ...
            // Stock's constructor installs the fixed "2 Orb Walkers" group.
            // Each Orb Walker constructor consumes two HP draws (the first is
            // overwritten), and no miscRng draw occurs at this boundary.
...
py::dict red_slaver_move_probe() {
    ...
    monster.moveHistory[0] = MMID::RED_SLAVER_SCRAPE;
    monster.moveHistory[1] = MMID::RED_SLAVER_ENTANGLE;

    py::dict result;
    for (const int roll : {49, 50, 54, 55, 74}) {
        int monster_data = monster.miscInfo;
        const auto move = monster.getMoveForRoll(battle, monster_data, roll);
        result[py::int_(roll)] = monsterMoveStrings[static_cast<int>(move)];
    }
    return result;
}
...
py::dict monster_move_parity_probe() {
    ...
    Monster acid;
    acid.id = MonsterId::ACID_SLIME_L;
    acid.moveHistory[0] = MMID::ACID_SLIME_L_CORROSIVE_SPIT;
    acid.moveHistory[1] = MMID::ACID_SLIME_L_CORROSIVE_SPIT;
    int acid_data = acid.miscInfo;
    const auto acid_move = acid.getMoveForRoll(acid_battle, acid_data, 0);
    result["acid_slime_l_a17_after_two_spits"] =
        monsterMoveStrings[static_cast<int>(acid_move)];

    BattleContext book_battle;
    book_battle.ascension = 18;
    Monster book;
    book.id = MonsterId::BOOK_OF_STABBING;
    int book_data = 1;
    const auto book_initial_move = book.getMoveForRoll(book_battle, book_data, 0);
    result["book_a18_initial_move"] =
        monsterMoveStrings[static_cast<int>(book_initial_move)];
    result["book_a18_initial_stab_count"] = book_data;

    book.moveHistory[0] = MMID::BOOK_OF_STABBING_MULTI_STAB;
    book.moveHistory[1] = MMID::BOOK_OF_STABBING_MULTI_STAB;
    book_data = 3;
    const auto book_after_two_move = book.getMoveForRoll(book_battle, book_data, 50);
    result["book_a18_after_two_multi_move"] =
        monsterMoveStrings[static_cast<int>(book_after_two_move)];
    result["book_a18_after_two_multi_stab_count"] = book_data;

    BattleContext automaton_battle;
    automaton_battle.ascension = 0;
    automaton_battle.player.curHp = 999;
    automaton_battle.player.maxHp = 999;
    automaton_battle.monsters.monstersAlive = 1;
    auto &automaton = automaton_battle.monsters.arr[0];
    automaton.id = MonsterId::BRONZE_AUTOMATON;
    automaton.idx = 0;
    automaton.curHp = 300;
    automaton.maxHp = 300;
    automaton.miscInfo = 1;  // State immediately after the summon turn.
    automaton.setMove(MMID::BRONZE_AUTOMATON_FLAIL);
    py::list automaton_sequence;
    automaton_sequence.append(
        monsterMoveStrings[static_cast<int>(automaton.moveHistory[0])]);
    for (int turn = 0; turn < 5; ++turn) {
        automaton.takeTurn(automaton_battle);
        while (!automaton_battle.actionQueue.isEmpty()) {
            auto action = automaton_battle.actionQueue.popFront();
            action(automaton_battle);
        }
        automaton_sequence.append(
            monsterMoveStrings[static_cast<int>(automaton.moveHistory[0])]);
    }
    result["bronze_automaton_after_summon"] = automaton_sequence;

    BattleContext orb_battle;
    Monster orb;
    orb.id = MonsterId::BRONZE_ORB;
    int orb_data = 0;
    const auto orb_first_move = orb.getMoveForRoll(orb_battle, orb_data, 25);
    result["bronze_orb_first_stasis_move"] =
        monsterMoveStrings[static_cast<int>(orb_first_move)];
    result["bronze_orb_used_stasis_on_selection"] = orb_data;
    const auto orb_next_move = orb.getMoveForRoll(orb_battle, orb_data, 25);
    result["bronze_orb_no_second_stasis_move"] =
        monsterMoveStrings[static_cast<int>(orb_next_move)];

    BattleContext nob_battle;
    nob_battle.ascension = 18;
    Monster nob;
    nob.id = MonsterId::GREMLIN_NOB;
    nob.moveHistory[0] = MMID::GREMLIN_NOB_BELLOW;
    nob.moveHistory[1] = MMID::INVALID;
    int nob_data = 0;
    result["gremlin_nob_a18_after_bellow"] = monsterMoveStrings[
        static_cast<int>(nob.getMoveForRoll(nob_battle, nob_data, 99))];
    nob.moveHistory[0] = MMID::GREMLIN_NOB_RUSH;
    nob.moveHistory[1] = MMID::GREMLIN_NOB_RUSH;
    result["gremlin_nob_a18_after_two_rushes"] = monsterMoveStrings[
        static_cast<int>(nob.getMoveForRoll(nob_battle, nob_data, 99))];

    BattleContext head_battle;
    head_battle.ascension = 18;
    head_battle.turn = 2;  // Selecting the move for monster turn four.
    Monster head;
    head.id = MonsterId::GIANT_HEAD;
    head.moveHistory[0] = MMID::GIANT_HEAD_COUNT;
    head.moveHistory[1] = MMID::GIANT_HEAD_GLARE;
    int head_data = 0;
    result["giant_head_a18_turn_four_move"] = monsterMoveStrings[
        static_cast<int>(head.getMoveForRoll(head_battle, head_data, 99))];
    head_battle.turn = 3;  // Executing the first It Is Time on turn four.
    head.setMove(MMID::GIANT_HEAD_IT_IS_TIME);
    result["giant_head_a18_first_time_damage"] =
        head.getMoveBaseDamage(head_battle).damage;

    BattleContext wizard_battle;
    wizard_battle.ascension = 0;
    wizard_battle.player.curHp = 999;
    wizard_battle.player.maxHp = 999;
    wizard_battle.monsters.monstersAlive = 1;
    auto &wizard = wizard_battle.monsters.arr[0];
    ...
    wizard_sequence.append(
        monsterMoveStrings[static_cast<int>(wizard.moveHistory[0])]);
    for (int turn = 0; turn < 4; ++turn) {
        wizard.takeTurn(wizard_battle);
        while (!wizard_battle.actionQueue.isEmpty()) {
            auto action = wizard_battle.actionQueue.popFront();
            action(wizard_battle);
        }
        wizard_sequence.append(
            monsterMoveStrings[static_cast<int>(wizard.moveHistory[0])]);
    }
    result["gremlin_wizard_sequence"] = wizard_sequence;

    BattleContext nemesis_battle;
    Monster nemesis;
    nemesis.id = MonsterId::NEMESIS;
    int nemesis_data = 0;
    const auto nemesis_initial = nemesis.getMoveForRoll(
        nemesis_battle, nemesis_data, 0);
    result["nemesis_initial_move"] =
        monsterMoveStrings[static_cast<int>(nemesis_initial)];
    result["nemesis_initial_cooldown"] = nemesis_data;
    nemesis.moveHistory[0] = MMID::NEMESIS_DEBUFF;
    nemesis.moveHistory[1] = MMID::NEMESIS_SCYTHE;
    nemesis_data = 1;
    const auto nemesis_after_one = nemesis.getMoveForRoll(
        nemesis_battle, nemesis_data, 0);
    result["nemesis_scythe_after_one_intervening_move"] =
        monsterMoveStrings[static_cast<int>(nemesis_after_one)];
    result["nemesis_reset_cooldown"] = nemesis_data;

    BattleContext snake_plant_battle;
    snake_plant_battle.ascension = 17;
    Monster snake_plant;
    snake_plant.id = MonsterId::SNAKE_PLANT;
    snake_plant.moveHistory[0] = MMID::SNAKE_PLANT_CHOMP;
    snake_plant.moveHistory[1] = MMID::SNAKE_PLANT_ENFEEBLING_SPORES;
    int snake_plant_data = 0;
    const auto snake_plant_move = snake_plant.getMoveForRoll(
        snake_plant_battle, snake_plant_data, 70);
    result["snake_plant_a17_after_recent_spores"] =
        monsterMoveStrings[static_cast<int>(snake_plant_move)];

    BattleContext champ_battle;
    champ_battle.ascension = 0;
    Monster champ;
    champ.id = MonsterId::THE_CHAMP;
    champ.curHp = 420;
    champ.maxHp = 420;
    champ.miscInfo = 2;  // forgeTimes has reached its stock cap.
    int champ_data = champ.miscInfo;
    const auto champ_capped_move = champ.getMoveForRoll(
        champ_battle, champ_data, 0);
    result["champ_after_two_defensive_stances"] =
        monsterMoveStrings[static_cast<int>(champ_capped_move)];

    champ.miscInfo = 0;
    champ.moveHistory[0] = MMID::THE_CHAMP_GLOAT;
    champ.takeTurn(champ_battle);
    result["champ_a0_gloat_strength"] = champ.getStatus<MS::STRENGTH>();

    BattleContext mass_battle;
    auto &mass = mass_battle.monsters.arr[0];
    mass.id = MonsterId::WRITHING_MASS;
    mass.idx = 0;
    mass.curHp = 160;
    mass.maxHp = 160;
    mass.setStatus<MS::REACTIVE>(0);
    mass.setHasStatus<MS::REACTIVE>(true);
    const auto reactive_rng_before = mass_battle.aiRng.counter;
    Actions::ReactiveRollMove().actFunc(mass_battle);
    result["writhing_mass_reactive_rng_draws"] =
        mass_battle.aiRng.counter - reactive_rng_before;
    result["writhing_mass_reactive_persists"] = mass.hasStatus<MS::REACTIVE>();
