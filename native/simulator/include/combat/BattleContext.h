...
namespace sts {
    ...
    struct BattleContext {
        ...
        // The native Gremlin Leader implementation reuses its three minion
        // slots.  Java's MonsterGroup retains replaced dead minions, so keep
        // display-only snapshots for the public/checkpoint bridge.
        std::array<Monster, 12> gremlinLeaderGhosts;
        std::array<int, 12> gremlinLeaderGhostSlots;
        int gremlinLeaderGhostCount = 0;

        // Collector and Reptomancer also replace a fixed native slot when
        // respawning a minion.  Java leaves the dead object in MonsterGroup
        // and inserts the replacement, so preserve those display/checkpoint
        // entities independently of the compact combat representation.
        std::array<Monster, 64> reusedSummonGhosts;
        std::array<int, 64> reusedSummonGhostSlots;
        int reusedSummonGhostCount = 0;
...
// ****************************************
        ...
        void chooseDiscardToHandCards(const fixed_list<int, 10> &idxs, bool forZeroCost);
        ...
        void recordGremlinLeaderGhost(const Monster &monster, int slot);
        void recordReusedSummonGhost(const Monster &monster, int slot);
        ...
        // card select screens
        void openDiscoveryScreen(std::array<CardId, 3> discoveryCards, int copyCount, CardType cardType);
    ...
    Action Actions::BuffPlayer(int amount) {
        return {[=] (BattleContext &bc) {
            ...
            if (s == PlayerStatus::MANTRA && bc.player.getStatus<PS::MANTRA>() >= 10) {
                const int remainder = bc.player.getStatus<PS::MANTRA>() - 10;
                if (remainder > 0) {
                    bc.player.setStatusValueNoChecks<PS::MANTRA>(remainder);
                } else {
                    bc.player.setHasStatus<PS::MANTRA>(false);
                    bc.player.statusMap.erase(PS::MANTRA);
                }
    ...
    Action Actions::BuffEnemy(int idx, int amount) {
        return {[=] (BattleContext &bc) {
            // ApplyPowerAction ignores a target that died between queueing and
            // execution.  Retaining a buff on a dead public monster corrupts
            // terminal/checkpoint parity.
