...
//        bool isDyingB = false;
        ...
        std::vector<MonsterStatus> powerOrder;
        ...
        int miscInfo = 0;
        // Curl Up remains visible on a monster killed by a later queued hit,
        // but it must not trigger a second time before its remove action runs.
        bool curlUpTriggered = false;
        ...
        [[nodiscard]] static int getPowerPriority(MonsterStatus s);
        void recordPowerApplied(MonsterStatus s);
        void recordPowerRemoved(MonsterStatus s);
        [[nodiscard]] std::vector<std::pair<MonsterStatus, int>> orderedPowers() const;
...
#pragma ide diagnostic ignored "cppcoreguidelines-narrowing-conversions"
    ...
    void Monster::setStatus(int amount) {
        ...
        if (s == MS::STRENGTH) {
            const bool wasActive = strength != 0;
            strength = amount;
            if (!wasActive && amount != 0) recordPowerApplied(s);
            if (wasActive && amount == 0) recordPowerRemoved(s);
            return;
        }
        setHasStatus<s>(amount != 0);
...
#pragma clang diagnostic pop
    ...
    void Monster::addDebuff(int amount, bool isSourceMonster) {
        ...
        switch (s) {
            ...
            case MonsterStatus::STRENGTH:
                if (strength == 0 && amount != 0) recordPowerApplied(s);
                strength += amount;
                setHasStatus<s>(true);
                return;
