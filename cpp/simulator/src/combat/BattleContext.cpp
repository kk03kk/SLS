...
void BattleContext::init(const GameContext &gc, MonsterEncounter encounterToInit) {
    ...
    actionQueue.clear();
    cardQueue.clear();
    gremlinLeaderGhostCount = 0;
    reusedSummonGhostCount = 0;
...
void BattleContext::initRelics(const GameContext &gc) {
    ...
    // Prismatic Shard's onEquip grants a non-Defect character one orb slot
    // when they had none. FullRun is Ironclad, whose base is zero; without
    // this, rewarded off-colour orb cards could never channel an orb.
    if (gc.cc != CharacterClass::DEFECT && gc.hasRelic(R::PRISMATIC_SHARD) &&
            p.orbSlots == 0) {
...
//                p.cardDrawPerTurn++;
            ...
            case R::SLAVERS_COLLAR:
                if (room == Room::ELITE || room == Room::BOSS) {
                    p.energyPerTurn++;
                }
                break;
...
//        canPlayCard = true; // what is this for......
        ...
        // CommunicationMod queues targeted cards directly and skips
        // AbstractPlayer.playCard(), so its protocol action does not turn the
        // player while Surrounded.  Preserve that observable action contract
        // for Original <-> Simulator differential runs.
        if (c.requiresTarget() && !player.hasStatus<PlayerStatus::SURROUNDED>()) {
...
void BattleContext::useNoTriggerCard() {
    ...
    // PlayTopCardAction may autoplay a status card (for example Havoc drawing
    // Burn).  The original moves it according to exhaustOnUse but does not
    // invoke the card's normal status effect.
    if (item.autoplay) {
        cards.removeFromHandById(c.uniqueId);
        const auto cardToMove = c;
        const bool exhaustOnUse = item.exhaustOnUse;
        addToBot({[cardToMove, exhaustOnUse](BattleContext &bc) {
            if (exhaustOnUse) {
                bc.triggerAndMoveToExhaustPile(cardToMove);
            } else {
                bc.cards.notifyRemoveFromHand(cardToMove);
                bc.cards.moveToDiscardPile(cardToMove);
            }
        }});
        return;
    }
...
//#endif // sts_asserts
    ...
    cards.removeFromHandById(c.uniqueId);
    // The queued cleanup must retain this card's own autoplay/exhaust flags.
    // A subsequent card-queue item can otherwise replace curCardQueueItem
    // before the cleanup runs (Havoc playing Burn is the observable case).
    const auto cardToMove = c;
    const bool exhaustOnUse = item.exhaustOnUse;
    addToBot({[cardToMove, exhaustOnUse](BattleContext &bc) {
        const bool spoonProc = exhaustOnUse &&
            bc.player.hasRelic<R::STRANGE_SPOON>() && bc.cardRandomRng.randomBoolean();
...
void BattleContext::useAttackCard() {
    ...
    switch (c.getId()) {
        ...
        case CardId::PERFECTED_STRIKE: {
            // Original countCards() scans hand, draw and discard only.  The
            // combat-wide strikeCount also includes exhaust and autoplay
            // limbo cards, which over-buffs Perfected Strike in both cases.
            const int strikeDmg = cards.countStrikeCardsInActivePiles() * (up ? 3 : 2);
        ...
        default:
            throw std::logic_error(
                std::string("unimplemented attack card: ") + c.getName());
    }
}
...
void BattleContext::useSkillCard() {
    ...
    switch (c.getId()) {
        ...
        case CardId::DARK_SHACKLES:
            ...
            // DarkShackles.use() checks Artifact before the queued
            // ApplyPowerAction executes.  Only an unprotected target receives
            // GainStrengthPower, which restores the lost Strength at end turn.
        ...
        default:
            throw std::logic_error(
                std::string("unimplemented skill card: ") + c.getName());
    }
}
...
void BattleContext::usePowerCard() {
    ...
    switch (c.getId()) {
        ...
        default:
            throw std::logic_error(
                std::string("unimplemented power card: ") + c.getName());
    }
    ...
    if (item.triggerOnUse) {
        ...
        if (m.hasStatus<MS::TIME_WARP>()) {
            ...
            if (timeWarp == 11) {
                ...
                // Time Warp remains installed after its counter resets. A zero
                // amount must not remove the power or later 12-card cycles can
                // never trigger again.
                m.setHasStatus<MS::TIME_WARP>(true);
                // TimeWarpPower adds per-monster ApplyPowerAction entries;
                // do not mutate a dead Time Eater at a terminal boundary.
                addToBot(Actions::BuffEnemy<MS::STRENGTH>(m.idx, 2));
        ...
        if (m.hasStatus<MS::SLOW>()) {
            // SlowPower.onAfterUseCard queues ApplyPowerAction.  The action
            // is intentionally skipped when the target was killed by the
            // card whose after-use hook is now running.
            addToBot(Actions::BuffEnemy<MS::SLOW>(m.idx, 1));
        }
...
void BattleContext::callEndOfTurnActions() {
    ...
    // AbstractPlayer walks its power list here, so keep the queued actions in
    // the order in which the powers were applied rather than a fixed enum
    // order. This becomes observable with two block-producing end-turn
    // powers and reactive effects such as Juggernaut.
    for (const auto [status, powerAmount] : player.orderedPowers()) {
        if (status == PS::METALLICIZE || status == PS::PLATED_ARMOR) {
            addToBot(Actions::GainBlock(powerAmount));
        } else if (status == PS::LIKE_WATER && player.stance == Stance::CALM) {
            addToBot(Actions::GainBlock(powerAmount));
        }
    }
    ...
    for (int i = 0; i < cards.cardsInHand; ++i) {
        ...
        c.triggerOnEndOfTurnForPlayingCard(*this);
    }
...
void BattleContext::applyEndOfRoundPowers() {
    ...
    player.applyAtEndOfRoundPowers(*this);
...
void BattleContext::triggerStartOfTurnOrbs() {
    for (int i = 0; i < player.orbSlots; ++i) {
        player.triggerStartOfTurnOrb(*this, i);
    }

    if (player.orbSlots > 0 && player.orbs[0] != Orb::EMPTY &&
            player.hasRelic<R::GOLD_PLATED_CABLES>()) {
        player.triggerStartOfTurnOrb(*this, 0);
    }
}
...
bool BattleContext::canUseSmokeBomb() const {
    // SmokeBomb.canUse() rejects every boss and any BackAttack source. The
    // compact Shield/Spear implementation represents BackAttack by Surrounded.
    if (player.hasStatus<PS::SURROUNDED>()) {
        return false;
    }
    switch (encounter) {
...
void BattleContext::drinkPotion(int idx, int target) {
    ...
    switch (p) {
        ...
        case Potion::EXPLOSIVE_POTION: {
            ...
            // Original Explosive Potion deliberately creates ownerless NORMAL
            // damage, so player attack hooks must not run here.
            addToBot( Actions::DamageAllEnemy(damage) );
            break;
        }
        ...
        case Potion::GAMBLERS_BREW:
            // GamblingChipAction is only queued by the original potion when
            // the hand is non-empty.  Opening an empty native selection
            // screen creates a UI boundary that does not exist in Java.
            if (cards.cardsInHand > 0) {
                addToBot( Actions::GambleAction() );
            }
            break;
...
void BattleContext::playTopCardInDrawPile(int monsterTargetIdx, bool exhausts) {
    ...
    // NewQueueCardAction carries status/curse cards with dontTriggerOnUseCard
    // set, so they resolve through the status-card path instead of silently
    // failing canUse().  Havoc autoplaying Burn is the parity-sensitive case.
    item.triggerOnUse = item.card.getType() != CardType::STATUS &&
        item.card.getType() != CardType::CURSE;
    // PlayTopCardAction creates NewQueueCardAction with immediateCard=false in
    // the original game.  That appends each autoplay card to the card queue.
    // Using pushFront reverses batches such as Distilled Chaos and changes the
    // point at which reactive powers (notably Guardian's Mode Shift) trigger.
    addToBotCard(item);
}

void BattleContext::recordGremlinLeaderGhost(const Monster &monster, int slot) {
    if (encounter != MonsterEncounter::GREMLIN_LEADER ||
            monster.id == MonsterId::INVALID ||
            gremlinLeaderGhostCount >= static_cast<int>(gremlinLeaderGhosts.size())) {
        return;
    }
    gremlinLeaderGhosts[gremlinLeaderGhostCount] = monster;
    gremlinLeaderGhostSlots[gremlinLeaderGhostCount] = slot;
    ++gremlinLeaderGhostCount;
}

void BattleContext::recordReusedSummonGhost(const Monster &monster, int slot) {
    if ((encounter != MonsterEncounter::COLLECTOR &&
            encounter != MonsterEncounter::REPTOMANCER) ||
            monster.id == MonsterId::INVALID ||
            reusedSummonGhostCount >= static_cast<int>(reusedSummonGhosts.size())) {
        return;
    }
    reusedSummonGhosts[reusedSummonGhostCount] = monster;
    reusedSummonGhostSlots[reusedSummonGhostCount] = slot;
    ++reusedSummonGhostCount;
}
...
void BattleContext::triggerOnOtherCardPlayed(const CardInstance &usedCard) {
    ...
    for (int i = 0; i < painCount; ++i) {
        // Pain queues LoseHPAction(player, player, 1).  It is therefore
        // self-inflicted HP loss and must notify Rupture.
...
void BattleContext::triggerAndMoveToExhaustPile(CardInstance c) {
    ...
    // DeadBranch.onExhaust suppresses its random generation after combat is
    // already effectively over.  Besides the invisible card, that guard also
    // prevents a cardRandomRng advance at a terminal boundary.
    if (player.hasRelic<R::DEAD_BRANCH>() && !monsters.areMonstersBasicallyDead()) {
        CardId id = getTrulyRandomCardInCombat(cardRandomRng, player.cc);
        addToBot(Actions::MakeTempCardInHand(id));
    }

    // DarkEmbracePower suppresses a draw once the fight is effectively over.
    // Keep the queued-action topology identical as terminal checkpoints retain
    // the underlying battle state.
    if (player.hasStatus<PS::DARK_EMBRACE>() && !monsters.areMonstersBasicallyDead()) {
    ...
    if (c.getId() == CardId::SENTINEL) {
        // Sentinel's triggerOnExhaust adds a top action.  Applying energy
        // immediately leaks it into terminal combat state and changes ordering
        // relative to other exhaust hooks.
        addToTop(Actions::GainEnergy(c.isUpgraded() ? 3 : 2));
    }
...
void BattleContext::mummifiedHandOnUsePower() {
    ...
    for (int i = matchingIdxList.size()-1; i >= 0; --i) {
        ...
        // Java runs relic hooks before GameActionManager removes the current
        // CardQueueItem, so the Power being played is excluded alongside
        // future queued cards.  The compact engine pops first and retains the
        // card in hand until cleanup, therefore it must exclude the current
        // item explicitly.
        if (uniqueId == curCardQueueItem.card.getUniqueId() ||
            cardQueue.containsCardWithId(uniqueId)) {
...
void BattleContext::openDiscoveryScreen(std::array<CardId, 3> discoveryCards, int copyCount, CardType cardType) {
    ...
    cardSelectInfo.discoveryCardType = cardType;
    // The original DiscoveryAction.update() generates its choices before the
...
void BattleContext::chooseForethoughtCards(const fixed_list<int, 10> &idxs) {
    ...
    for (const auto handIdx : idxs) {
        selected.push_back(cards.hand[handIdx]);
    }
