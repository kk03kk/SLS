...
int CardManager::countStrikeCardsInActivePiles() const {
    int count = 0;
    for (int i = 0; i < cardsInHand; ++i) {
        count += hand[i].isStrikeCard();
    }
    for (const auto &card : drawPile) {
        count += card.isStrikeCard();
    }
...
void CardManager::draw(BattleContext &bc, int amount) {
    for (int i = 0; i < amount; i++) {
        ...
        if (c.getType() == CardType::SKILL) {
            ...
            // VoidCard.triggerWhenDrawn queues LoseEnergyAction before
            // AbstractPlayer subsequently notifies onCardDraw powers.
            if (c.getId() == CardId::VOID) {
                bc.addToBot(Actions::LoseEnergy(1));
            }
            // AbstractPlayer.draw invokes onCardDraw on powers in their
            // application order. Evolve and Fire Breathing can therefore
            // enqueue in either order; a fixed Evolve-then-Fire ordering
            // changes action timing when the powers were played oppositely.
            for (const auto [status, powerAmount] : bc.player.orderedPowers()) {
                if (status == PS::EVOLVE) {
                    bc.addToBot(Actions::DrawCards(powerAmount));
                } else if (status == PS::FIRE_BREATHING) {
                    bc.addToBot(Actions::DamageAllEnemy(powerAmount));
                }
            }
        } else if (c.getType() == CardType::CURSE) {
            for (const auto [status, powerAmount] : bc.player.orderedPowers()) {
                if (status == PS::FIRE_BREATHING) {
                    bc.addToBot(Actions::DamageAllEnemy(powerAmount));
                }
            }
