...
void MonsterGroup::createMonsters(BattleContext &bc, MonsterEncounter encounter) {
    switch (encounter) {
        ...
        case MonsterEncounter::JAW_WORM_HORDE: {
            ...
            // JawWorm.bellowBlock is 6 at every ascension below 17.  The horde
            // constructs all three worms in hard mode, so their pre-battle
            // block uses the same value as Bellow rather than Thrash's 5.
            const auto blockBuff = bc.ascension >= 17 ? 9 : 6;
            for (int i = 0; i < 3; ++i) {
...
void MonsterGroup::applyEmeraldEliteBuff(BattleContext &bc, const int buffType, const int act) {
    for (int i = 0; i < monsterCount; ++i) {
        ...
        switch (buffType) {
            case 0:
                // MonsterRoomElite.java uses AbstractDungeon.actNum + 1.
                m.buff<MS::STRENGTH>(act + 1);
                break;
