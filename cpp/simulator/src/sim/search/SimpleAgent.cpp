...
fixed_list<int,16> search::SimpleAgent::getBestMapPathForWeights(
        const Map &m, const int *weights, bool prioritizeBurningElite) {
    const auto nodeWeight = [&](const MapNode &node) {
        const int keyWeight = prioritizeBurningElite &&
            node.x == m.burningEliteX && node.y == m.burningEliteY ? 100000 : 0;
        return weights[static_cast<int>(node.room)] + keyWeight;
    };
    Path paths1[7];
    Path paths2[7];
    ...
    for (int x = 0; x < 7; ++x) {
        ...
        if (node.edgeCount > 0) {
            ...
            paths1[x].weight = nodeWeight(node);
        }
    }
    ...
    for (int y = 0; y < 14; ++y) {
        ...
        for (int x = 0; x < 7; ++x) {
            ...
            for (int i = 0; i < node.edgeCount; ++i) {
                ...
                const int roomWeight = nodeWeight(m.getNode(edge, y+1));
...
bool search::SimpleAgent::playPotion(BattleContext &bc) {
    ...
    for (; i < bc.potionCapacity; ++i) {
        ...
        if (canDrink) {
            ...
            const auto action = search::Action(search::ActionType::POTION, i, target);
...
void search::SimpleAgent::stepBattleCardSelect(BattleContext &bc) {
    ...
    if (bc.cardSelectInfo.cardSelectTask == CardSelectTask::LIQUID_MEMORIES_POTION &&
            bc.cardSelectInfo.pickCount > 1) {
        fixed_list<int, 10> selected;
        for (int i = 0; i < bc.cardSelectInfo.pickCount; ++i) {
            selected.push_back(actions[i].first.getSelectIdx());
        }
...
void search::SimpleAgent::stepOutOfCombat(GameContext &gc) {
    switch (gc.screenState) {
        ...
        case ScreenState::MAP_SCREEN: {
            ...
            if (gc.curMapNodeY < 0) {
                mapPath = getBestMapPathForWeights(
                    *gc.map, mapWeights[gc.act-1], !gc.hasKey(Key::EMERALD_KEY));
            }
...
void search::SimpleAgent::stepRewardsScreen(GameContext &gc) {
    ...
    // Gold is an independent combat reward in Slay the Spire: proceeding from
    // the screen does not collect it.  Take every stack before choosing any
    // optional reward so the scripted FullRun policy models the real run.
    while (r.goldRewardCount > 0) {
        takeAction(gc, {GameAction::RewardsActionType::GOLD, 0});
    }
    ...
    // Bottled relics open a card-selection screen while the reward action is
    // executing.  Do not append a rewards action against that new screen;
    // return to the main playout loop so the selection itself is recorded.
