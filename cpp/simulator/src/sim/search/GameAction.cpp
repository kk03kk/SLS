...
bool isValidMatchAndKeepEventAction(const GameContext &gc, const search::GameAction a) {
    ...
    return idx1 >= 0 && idx1 < gc.info.toSelectCards.size() &&
        idx2 > idx1 && idx2 < gc.info.toSelectCards.size() &&
        gc.info.toSelectCards[idx1].deckIdx != 0 &&
        gc.info.toSelectCards[idx2].deckIdx != 0;
}
...
bool isValidMapAction(const GameContext &gc, const search::GameAction a) {
    ...
    if (gc.hasRelic(RelicId::WING_BOOTS)
        && gc.relics.getRelicValue(RelicId::WING_BOOTS) > 0) {
        return gc.map->getNode(select, gc.curMapNodeY + 1).room != Room::NONE;
    }
    return false;
}
...
void executeRewardsAction(GameContext &gc, const search::GameAction a) {
    ...
    switch (a.getRewardsActionType()) {
        ...
        case search::GameAction::RewardsActionType::RELIC:
        {
            const bool tookSapphireRelic =
                r.sapphireKey && a.getIdx1() == r.relicCount - 1;
            gc.obtainRelic(r.relics[a.getIdx1()]);
            r.removeRelicReward(a.getIdx1());
            if (tookSapphireRelic) {
                r.sapphireKey = false;
            }
            break;
        }
...
void search::GameAction::execute(GameContext &gc) const {
    ...
    switch (gc.screenState) {
        ...
        case ScreenState::MAP_SCREEN:
            if (gc.curMapNodeY >= 0 && gc.curMapNodeY < 14
                && gc.hasRelic(RelicId::WING_BOOTS)) {
                const auto &node = gc.map->getNode(gc.curMapNodeX, gc.curMapNodeY);
                bool naturallyConnected = false;
                for (int i = 0; i < node.edgeCount; ++i) {
                    naturallyConnected |= node.edges[i] == getIdx1();
                }
                if (!naturallyConnected) {
                    auto &charges = gc.relics.getRelicValueRef(RelicId::WING_BOOTS);
                    charges = charges <= 1 ? -2 : charges - 1;
                }
            }
...
std::vector<search::GameAction> getAllMapActions(const sts::GameContext &gc) {
    ...
    if (gc.curMapNodeY == 14) {
        ...
        for (int i = 0; i < node.edgeCount; ++i) {
            ...
            added.set(node.edges[i]);
        }
        if (gc.hasRelic(RelicId::WING_BOOTS)
            && gc.relics.getRelicValue(RelicId::WING_BOOTS) > 0) {
            for (int x = 0; x < 7; ++x) {
                if (!added.test(x)
                    && gc.map->getNode(x, gc.curMapNodeY + 1).room != Room::NONE) {
            ...
            if (gc.curEvent == Event::MATCH_AND_KEEP) {
                ...
                for (int first = 0; first < gc.info.toSelectCards.size(); ++first) {
                    ...
                    for (int second = first + 1; second < gc.info.toSelectCards.size(); ++second) {
                        if (gc.info.toSelectCards[second].deckIdx != 0) {
                            actions.emplace_back(first, second);
                        }
                    }
                }
                return actions;
