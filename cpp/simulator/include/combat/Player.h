...
namespace sts {
    ...
    struct Player {
        ...
        int8_t orbSlots = 0;
        static constexpr int MAX_ORB_SLOTS = 10;
        std::array<Orb, MAX_ORB_SLOTS> orbs{};
        std::array<std::int16_t, MAX_ORB_SLOTS> orbEvokeAmounts{};
        ...
        // AbstractCreature.powers is stably sorted by AbstractPower.priority.
        // Keep acquisition order separately from the compact value storage so
        // equal-priority callbacks remain deterministic and checkpointable.
        std::vector<PlayerStatus> powerOrder;
        ...
        [[nodiscard]] static int getPowerPriority(PlayerStatus s);
        void recordPowerApplied(PlayerStatus s);
        void recordPowerRemoved(PlayerStatus s);
        [[nodiscard]] std::vector<std::pair<PlayerStatus, int>> orderedPowers() const;
        ...
        void triggerStartOfTurnOrb(BattleContext &bc, int idx);
        void triggerEndOfTurnOrb(BattleContext &bc, int idx);
        void triggerEndOfTurnOrbs(BattleContext &bc);
        void adjustOrbFocus(int amount);
        [[nodiscard]] bool hasEmptyOrb() const;

        void applyEndOfTurnPowers(BattleContext &bc);
        void applyAtEndOfRoundPowers(BattleContext &bc);
...
// intangible
    ...
    void Player::decrementStatus(int amount) {
        switch (s) {
            case PS::ARTIFACT:
                artifact -= amount;
                if (artifact == 0) recordPowerRemoved(s);
                break;

            case PS::DEXTERITY:
                dexterity -= amount;    // dexterity should not be used by "hasStatusInternal"
                if (dexterity == 0) recordPowerRemoved(s);
                break;

            case PS::FOCUS:
                focus -= amount;
                adjustOrbFocus(-amount);
                if (focus == 0) recordPowerRemoved(s);
                break;

            case PS::STRENGTH:
                strength -= amount;     // strength should not be used by "hasStatusInternal"
                if (strength == 0) recordPowerRemoved(s);
                break;
...
//        static_assert(s != PlayerStatus::THE_BOMB);
        ...
        const bool wasActive = hasStatus<s>();
        ...
        if (value && !wasActive) recordPowerApplied(s);
        if (!value && wasActive) recordPowerRemoved(s);
    }
...
// intangible
    ...
    void Player::buff(int amount) {
        ...
        switch (s) {
            case PS::ARTIFACT:
                artifact += amount;
                if (!wasActive && artifact != 0) recordPowerApplied(s);
                return;

            case PS::DEXTERITY:
                dexterity += amount;
                if (!wasActive && dexterity != 0) recordPowerApplied(s);
                return;

            case PS::FOCUS:
                focus += amount;
                adjustOrbFocus(amount);
                if (!wasActive && focus != 0) recordPowerApplied(s);
                return;

            case PS::STRENGTH:
                strength += amount;
                if (!wasActive && strength != 0) recordPowerApplied(s);
                return;
        ...
        // PanachePower starts with its five-card counter immediately when it
        // is applied (not only at the next turn start). Stacking changes
        // damage but must not reset an in-progress counter.
    ...
    void Player::setStatusValueNoChecks(int value) {
        ...
        if (!wasActive && value != 0) recordPowerApplied(s);
        if (wasActive && value == 0) recordPowerRemoved(s);
    }
